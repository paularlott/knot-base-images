...
Minimal [Alpine](https://hub.docker.com/_/alpine) with the knot toolchain — the `knot-entrypoint`, runtime user handling, `rsyslog` logging, startup hooks and cron — but **no development tools**: no ssh, git, editors, shells or database clients. It is the base for [`knot-alpine`](https://hub.docker.com/r/paularlott/knot-alpine) (which adds the dev toolchain) and for lean runtime images such as [`knot-scriptling-runtime`](https://hub.docker.com/r/paularlott/knot-scriptling-runtime).
...
All tags are multi-arch (`linux/amd64`, `linux/arm64`); see [Docker Hub](https://hub.docker.com/r/paularlott/knot-alpine-runtime/tags) for the current list. Each release is also tagged `<version>-<BUILD_DATE>`.
...
docker run --rm \
  -e KNOT_USER=alice \
  -v knot_home:/home \
  paularlott/knot-alpine-runtime:3.24 \
  sh -c 'echo hello from $(whoami)'
```
...
| `KNOT_SSHD` | _(unset)_ | Set to `native` to start `sshd` (not installed in this image; use `knot-alpine`) |
